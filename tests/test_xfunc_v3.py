"""Driver: QTT function approximation via DMRG and cross local solvers.

Approximates a 1-D analytic function as a quantized tensor train using both the
DMRG and cross-interpolation local evaluators and compares their accuracy
...
if do_test_4:
    ...
    for D in Ds:
        ...
        term1c = Term_Cross(init_mps.data.copy(), operators=[op.data.copy()], num_tiers=2)
...
if do_test_6:
    ...
    term1d = Term_DMRG(init_mps.data.copy(), operators=[op.data.copy()], mps_power=power, mps_coeff=coeff, num_tiers=2)
                        #, num_tiers=2)     ## this doesn't work; inconsistent bond dimensions.
    term1c = Term_Cross(init_mps.data.copy(), operators=[op.data.copy()], mps_func=func, num_tiers=2)
...
Ds[-1] = 100
i = 0
for errs1, errs2, errs3 in zip(dmrg_errs, cross_errs, comp_errs):
    plt.figure()
    plt.semilogy(Ds, errs1, 'o-', label='SVD')
    plt.semilogy(Ds, errs2, 'x--', label='CUR')
    plt.semilogy(Ds, errs3, 'v:', label='global')
    plt.legend()
    plt.xlabel('QTT rank')
    plt.ylabel('error (L2)')
    plt.title(legs[i])
