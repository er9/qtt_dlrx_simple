...
qtt_pos = grid.map_state_to_mps(data + offset, split_opts={'max_bond': max_rank, 'cutoff': tol**2})
...
print('distance', np.linalg.norm(qtt.get_data() - data)/np.linalg.norm(data))

for t in qtt_pos.data.tensors:
    print('neg value count', np.sum(t.data < 0))


print('POS QTT')
print('max rank', qtt_pos.max_bond())
print('internal bond sizes', helper.inner_bond_sizes(qtt_pos.data))
print('entanglement entropy at all bonds', helper.entanglement_entropy_all(qtt_pos.data))
print('distance', np.linalg.norm((qtt_pos.get_data() - offset) - data) / np.linalg.norm(data))
